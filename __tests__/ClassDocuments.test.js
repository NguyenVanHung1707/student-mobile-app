...
describe('ClassDocuments Screen (Student Permissions Validation)', () => {
  ...
  const mockDocuments = [
    ...
    { id: 11, name: 'Baitap.pdf', type: 'FILE', fileExtension: 'pdf', fileSize: 102400, uploaderName: 'GV Nguyễn Văn B' }
  ];

  const mockFolderContents = [
    { id: 20, name: 'Chuong_1_Gioi_Thieu.pdf', type: 'FILE', fileExtension: 'pdf', fileSize: 50240, uploaderName: 'GV Nguyễn Văn B' }
  ];
  ...
  it('renders explorer loading and shows empty list when no docs', async () => {
    ...
    axios.get.mockImplementation((url) => {
      if (url.includes('/my-permissions')) {
        return Promise.resolve({ data: { canUploadDocuments: false, canDownloadDocuments: false } });
      }
  ...
  it('shows upload button if canUploadDocuments is true', async () => {
    axios.get.mockImplementation((url) => {
      if (url.includes('/my-permissions')) {
        return Promise.resolve({ data: { canUploadDocuments: true, canDownloadDocuments: false } });
      }
      return Promise.resolve({ data: mockDocuments });
    });
  ...
  it('shows permission toast warning when clicking download without canDownloadDocuments', async () => {
    ...
    expect(toastSpy).toHaveBeenCalledWith('Bạn chưa được cấp quyền tải tài liệu!', ToastAndroid.SHORT);
  });

  it('navigates through nested directories and updates breadcrumbs correctly', async () => {
    axios.get.mockImplementation((url) => {
      if (url.includes('/my-permissions')) {
        return Promise.resolve({ data: { canUploadDocuments: false, canDownloadDocuments: true } });
      }
